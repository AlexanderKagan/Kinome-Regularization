...
train_input_path <- paste(input_path, "train.csv", sep = "")
test_input_path <- paste(input_path, "test.csv", sep = "")
train_data <- read.csv(train_input_path)
test_data <- read.csv(test_input_path)
...
fit_vbvs = vbvs_concurrent(model, id.var = "subj", data = train_data, t.min = 0.0, t.max = 10.0, standardized = FALSE)

# write.csv(fit_vbvs$spline.coef.est, "./fof/data/r/result_coefficients.csv", row.names=FALSE)
# write.csv(fit_vbvs$beta.pm, "./fof/data/r/result_beta.csv", row.names=FALSE)
# plot_shiny(fit_vbvs)

predicted <- predict(fit_vbvs, data.new = test_data, standardized = FALSE)

y_predicted_output_path <- paste(output_path, "y_predicted.csv", sep = "")
y_true_output_path <- paste(output_path, "y_true.csv", sep = "")

write.csv(predicted, y_predicted_output_path, row.names=FALSE)
write.csv(fit_vbvs$data.model$Y, y_true_output_path, row.names=FALSE)
