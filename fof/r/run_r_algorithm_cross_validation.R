...
CV_regressor <- function(data) {
  ...
  for (drug in unique(drug_names_matrix)) {
    ...
    fit_vbvs = vbvs_concurrent(model, id.var = "subj", data = data_train, t.min = 0.0, t.max = 10.0, standardized = FALSE)
    vb_concurrent
    betas_path <- paste("./fof/data/r/cross_validation_output/result_beta", drug, sep = "")
    betas_path <- paste(betas_path, ".csv", sep = "")
...
final_result <- CV_regressor(data)
y_pred_final <- final_result[[1]]
y_true_standartized_final <- final_result[[2]]

write.csv(y_pred_final, 'fof/data/r/cross_validation_output/y_pred_final.csv', row.names=FALSE)
write.csv(y_true_standartized_final, 'fof/data/r/cross_validation_output/y_true_standartized_final.csv', row.names=FALSE)