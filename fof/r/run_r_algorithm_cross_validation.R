...
CV_regressor <- function(data, v0) {
  ...
  for (drug in unique(drug_names_matrix)) {
    ...
    fit_vbvs = vbvs_concurrent(model, id.var = "subj", data = data_train, t.min = 0.0, t.max = 10.0, standardized = FALSE, v0 = v0)

    betas_path <- paste("./fof/data/r/cross_validation_output/result_beta_", drug, "_", v0, ".csv", sep = "")
    ...
    y_pred_final[test_index] <- y_test_predicted
    y_true_standartized_final[test_index] <- y_test_true_standartized

  }
  return (list(y_pred_final, y_true_standartized_final))
}

for (v0 in c(0.001, 0.003, 0.1, 0.3, 1, 3, 10, 30)){
  print("ITERATION")
  print(v0)
  final_result <- CV_regressor(data, v0)
  y_pred_final <- final_result[[1]]
  y_true_standartized_final <- final_result[[2]]

  y_pred_final_path <- paste("./fof/data/r/cross_validation_output/y_pred_final_", v0, ".csv", sep = "")
  write.csv(y_pred_final, y_pred_final_path, row.names=FALSE)
  y_true_standartized_final_path <- paste("./fof/data/r/cross_validation_output/y_true_standartized_final_", v0, ".csv", sep = "")
  write.csv(y_true_standartized_final, y_true_standartized_final_path, row.names=FALSE)
}