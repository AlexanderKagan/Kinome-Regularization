...
# fit_vb = vb_concurrent(Y ~ Cov_1 + Cov_2 + Cov_3 | time, id.var = "subj", data = data,
#                        t.min = 1, t.max = 11, standardized = TRUE)
# plot_shiny(fit_vb)
...
fit_vbvs = vbvs_concurrent(model, id.var = "subj", data = data,
                           t.min = 0.0, t.max = 10.0, standardized = FALSE)
...
predicted <- predict(fit_vbvs, data.new = data, standardized = FALSE)
# same as Yhat from fit_vbvs

write.csv(predicted, "./fof/data/r/result_y.csv", row.names=FALSE)
write.csv(fit_vbvs$data.model$Y, "./fof/data/r/result_y_true.csv", row.names=FALSE)
