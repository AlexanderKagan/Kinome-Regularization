...
CV_regressor <- function(data, v0, train_index, test_index) {
  ...
  A <- 0.1
  B <- 0.1
  fit_vbvs = vbvs_concurrent(model, id.var = "subj", Kp = 7, Kt = 4, data = data_train, t.min = 0.0, t.max = 10.0, standardized = FALSE, v0 = v0, A = A, B = B, Atheta = A, Btheta = B)

  betas_path <- paste("./fof/data/r/validation_output_new/result_beta_", "train", "_", v0, ".csv", sep = "")
...
# c(0.001, 0.003, 0.1, 0.3, 1, 3, 10, 30)
# c(0.000001,0.00001, 0.0001, 60, 80, 100)
# 0.05
for (v0 in c(1)){
  print("ITERATION")
  print(v0)
  final_result <- CV_regressor(data, v0, train_index, test_index)
  ...
  y_train_predicted_path <- paste("./fof/data/r/validation_output_new/y_train_predicted_", v0, ".csv", sep = "")
  write.csv(y_train_predicted, y_train_predicted_path, row.names=FALSE)

  y_train_true_standartized_path <- paste("./fof/data/r/validation_output_new/y_train_true_standartized_", v0, ".csv", sep = "")
  write.csv(y_train_true_standartized, y_train_true_standartized_path, row.names=FALSE)

  y_test_predicted_path <- paste("./fof/data/r/validation_output_new/y_test_predicted_", v0, ".csv", sep = "")
  write.csv(y_test_predicted, y_test_predicted_path, row.names=FALSE)

  y_test_true_standartized_path <- paste("./fof/data/r/validation_output_new/y_test_true_standartized_", v0, ".csv", sep = "")
